...
└─────────────────────────────────────────────────────────┘
        ...
        ┌────────────────┴────────────────┐
        ↓                                  ↓
  [Agent A 本地工作区]              [Agent B 本地工作区]
  E:\...\vision-electronic-        /tmp/worktree-kub/
  contest-/                         ...
        │                                  │
        └───────────── 工作完成后 push ──────┘
```
...
- **计划**：以 GitHub 仓库里的 `整改计划.md` 和 `AI工作文档.md` 为准
- **报告**：写入本地 `reports/` 目录，完成后随代码一起推送
...
本项目按 `整改计划.md` 中的 Phase 顺序推进。每次工作时：

1. 从 GitHub 拉取最新代码：`git pull`
2. 查看上方"当前工作状态"，确认从哪个子任务开始
3. 领取方式：在本文件的"当前工作状态"栏更新进度（注明"由 [你的名字/Agent ID] 领取"）
4. 按下方"单次任务工作流程"执行
...
步骤 2 → 阅读任务定义
          读取 整改计划.md 中对应 Phase 的具体任务描述（T1.x / T2.x 等）

步骤 3 → 理解相关代码
          读取任务直接涉及的源代码文件（通常 1-3 个）
          确认改动范围，不改变任务范围外的逻辑

步骤 4 → 改动前记录
          对要改动的文件，先 git add + git commit 记录改动前状态

步骤 5 → 实施改动
          按任务要求创建/修改文件

步骤 6 → 验证改动
          运行相关示例脚本或测试，确认功能等价于改动前

步骤 7 → 填写工作报告
          按下方"六、工作报告模板"填写
          写入 reports/[任务编号]-[日期].md
          例：reports/T1.3-20250523.md

步骤 8 → 提交并推送
          git add + git commit（信息格式：[T{x}.{y}] {简短描述}）
          git push
...
**文件命名规则**：`[任务编号]-[日期].md`，例：`T1.3-20250523.md`
...
1. **功能等价优先**：任何重构不得改变检测结果。改动前后用同一个测试视频（`test_video.avi`）验证，召回率不得下降。
...
4. **不写冗余注释**：代码本身能说明的不要加注释，只在非显而易见的逻辑处加说明。
...
```
1. 本文件（AI工作文档.md）← 你现在在这里，搞清楚项目是什么
2. 整改计划.md 的"六、重构顺序" ← 了解总体执行路径
3. 整改计划.md 的对应 Phase   ← 了解你要做的那个 Phase 的具体任务
4. 相关源代码文件              ← 开始工作
```
...
├── tests/                       # （Phase 4 新建）
├── main.py
├── requirements.txt              # （Phase 1 新建）
├── .gitignore                    # （Phase 1 新建）
└── ...
```

> **注意**：所有 Phase 1-4 的改动都将在 GitHub 仓库中体现，请勿在本地创建与仓库平行的"第二套"文档体系。
